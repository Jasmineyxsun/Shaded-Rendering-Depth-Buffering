// Cylinder3D to display 3D cylinder with cap 
// author: Yuxin Sun
...
public class Cylinder3D
{
	...
	public Cylinder3D(float _x, float _y, float _z, float _rx, float _ry, int _m, int _n, float _r)
	{
	...
	for (i = 0, theta = vmin; i < m; ++i, theta += d_theta){
		...
		mesh.v[i][0] = new Vector3D(center.x,center.y,center.z - (float)r);
		
		//front endcap
		mesh.n[i][n-1] = new Vector3D(c_theta,s_theta,1);
		mesh.n[i][n-1].normalize();
		mesh.v[i][n-1] = new Vector3D(center.x,center.y,center.z + (float)r);
	}
}
}